...
public class QrcodeUtils {
     ...
     * @param config   logo 显示配置：边框大小、颜色
     * @return 二维码图片的字节数组
     */
    public static byte[] createQRCode(String content, int length, File logoFile, MatrixToLogoImageConfig config) {
        ...
        config = (null == config) ? new MatrixToLogoImageConfig() : config;
        try {
            ...
            if (logoFile != null) {
                ...
                overlapImage(img, FORMAT, file.getAbsolutePath(), logoFile, config);
            }
    ...
    public static byte[] createQRCode(String content, int length, File logoFile) {
        return createQRCode(content, length, logoFile, null);
    }
     ...
     * @return 二维码图片的字节数组
     */
    public static byte[] createQRCode(String content, File logoFile, MatrixToLogoImageConfig config) {
        return createQRCode(content, DEFAULT_LENGTH, logoFile, config);
    }
    ...
    public static String decodeQrcode(File file) throws IOException, NotFoundException {
        ...
        hints.put(DecodeHintType.CHARACTER_SET, Charset.forName("UTF-8").name());
    ...
    public static void main(String[] args) {
        ...
        URL url = ClassLoader.getSystemResource("images/logo_hxdf.jpg");
        if (null == url) {
            logger.error("logo文件不存在");
            return;
        }
        File logFile = new File(url.getPath());
        MatrixToLogoImageConfig config = new MatrixToLogoImageConfig(0,Color.LIGHT_GRAY);
        byte[] qrByte = createQRCode("https://api-park-test.hejiahuan.cn/", logFile, config);
        try {
            FileUtils.writeByteArrayToFile(qrFile, qrByte);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
